...
bool HardwareNode::Open() {
  ...
  state_pub_ = std::make_unique<ipc::Publisher<talos::hardware::Packet>>("/hw/state");
  cmd_pub_ = std::make_unique<ipc::Publisher<talos::hardware::Packet>>("/hw/cmd");
  ds_pub_ = std::make_unique<ipc::Publisher<talos::hardware::Packet>>("/hw/ds");

  for (auto& [name, tracker] : subsystems_) {
    std::string topic = "/hw/req/" + name;
    tracker.subscriber = std::make_unique<ipc::Subscriber<talos::hardware::Packet>>(
