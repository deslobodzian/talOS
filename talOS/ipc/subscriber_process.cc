...
int main(int argc, char** argv) {
  ...
  std::vector<std::chrono::nanoseconds> read_durations;
  ...
  while (Clock::now() < run_duration) {
    ...
    auto msg = sub.read();
    if (msg) {
      auto fb_msg = msg.value();
      auto stop = Clock::now();

      auto read_duration =
          std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start);

      auto sent_time = Clock::time_point{std::chrono::nanoseconds{
          static_cast<std::chrono::nanoseconds::rep>(fb_msg.value())}};
  ...
  calculate_metrics(read_durations);
