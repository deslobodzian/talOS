...
class RawBuffer {
 public:
  RawBuffer(Device& device, BufferDesc desc) : device_(&device), desc_(desc) {
...
class Buffer {
 public:
  Buffer(Device& device, std::size_t count,
      ...
      : count_(count),
        raw_(device, BufferDesc{.size = count * sizeof(T),
                                .location = location,
                                .source = MemorySource::Allocate,
                                .external_ptr = nullptr}) {}

  Buffer(Device& device, T* external_ptr, std::size_t count,
  ...
  T* data() const { return static_cast<T*>(raw_.data()); }
